...
$hotels[0]
  ->create_related('manager', {manager_num_b => 5555555, name => 'Lalolu'});
my @managers = Manager->new->find();
is($ENV{OBJECTDB_BUILT_SCHEMAS},        2);
is($managers[0]->column('hotel_num_b'), $hotels[0]->column('hotel_num_a'));


# "create_related" data via class that hasnt been loaded so far
$managers[0]->create_related('telefon_numbers',
    [{tel_num_c => 1111, telefon_number => '123456789'}]);
is($ENV{OBJECTDB_BUILT_SCHEMAS}, 3);
my @tel_nums = TelefonNumber->new->find;
is( $tel_nums[0]->column('manager_num_c'),
    $managers[0]->column('manager_num_b')
);
...
# "delete"
use Tag;
Tag->new->set_columns(id => 1)->delete;
is($ENV{OBJECTDB_BUILT_SCHEMAS}, 11);


# _resolve_where
@hotels = Hotel->new->find(where => ['apartments.size' => 4]);
is($ENV{OBJECTDB_BUILT_SCHEMAS}, 12);


# prefetch
my @apartments = Apartment->new->find(with => 'rooms');
